...
sub BUILD
{
	...
	Carp::croak("Error. You must call new as new(input_file => 'some file'). ") if (! $self -> input_file);
	...
	if ($g =~ /^(\d*)(x?)(\d*)([+-]?\d*)([+-]?\d*)$/i)
	{
		...
		die "Error. Input (NxM+x+y = $g) specifies N = 0. \n"	if ($g[0] =~ /^0+$/);
		die "Error. Input (NxM+x+y = $g) specifies M = 0. \n"	if ($g[2] =~ /^0+$/);
		...
		if ($self -> verbose)
		{
			...
			print "Geometry:             $g parsed as NxM+x+y = " . join('', @{$self -> geometry }) . ". \n";
		}
	}
	else
	{
...
sub tile
{
	...
	$$param{tile}									= {};
	$$param{tile}{width}							= int($$param{image}{width} / $self -> geometry[0]);
	$$param{tile}{height}							= int($$param{image}{height} / $self -> geometry[2]);
	...
	die "Error. Tile width ($$param{tile}{width}) < input x ($g[3]). \n"	if ($$param{tile}{width} < abs($g[3]) );
	die "Error. Tile height ($$param{tile}{height}) < input y ($g[4]). \n"	if ($$param{tile}{height} < abs($g[4]) );

	$$param{tile}{width}	+= $g[3];
	$$param{tile}{height}	+= $g[4];
	...
	for my $xg (1 .. $self -> geometry[0])
	{
		$y = 0;

		for my $yg (1 .. $self -> geometry[2])
		{
